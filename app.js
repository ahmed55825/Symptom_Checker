...
app.get("/admin/view-doctors", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      res.render("tables/view_doctors_table", { empty: true });
    } else {
      res.render("tables/view_doctors_table", {
        doctors: results,
        empty: false,
      });
    }
...
app.get("/admin/view-users", async (req, res) => {
  ...
  try {
    ...
    if (results.length === 0) {
      res.render("tables/view_users_table", { empty: true });
    } else {
      res.render("tables/view_users_table", { users: results, empty: false });
    }
  } catch (err) {
    console.log(err);
    res.redirect("/log-in");
...
app.post("/admin/new_doctor", async (req, res) => {
  ...
  `;
    ...
    res.status(201).redirect("/admin_view");
  } catch (error) {
    console.error(error);
    res.redirect("/log-in");
  }
...
app.post("/admin/update_doctor", async (req, res) => {
  const { doctor_id, new_doctor_name, doctor_age } = req.body;

  console.log(doctor_id, new_doctor_name, doctor_age);
  ...
  `;
    ...
    await new Promise((resolve, reject) => {
      connection.query(
        updateQuery,
        [new_doctor_name, doctor_age, doctor_id],
        (err, results) => {
          if (err) {
...
app.post("/admin/update_user", async (req, res) => {
  const { user_id, new_name, new_age, new_cnic, new_phone } = req.body;

  console.log(user_id, new_name, new_age, new_cnic, new_phone);

  try {
    const updateQuery = `
      UPDATE user
      SET Name = ?, Age = ?, CNIC = ?, Phone = ?
      WHERE user_id = ?;
    `;
    ...
    await new Promise((resolve, reject) => {
      connection.query(
        updateQuery,
        [new_name, new_age, new_cnic, new_phone, user_id],
        (err, results) => {
          if (err) {
...
app.post("/admin/delete_doctor", async (req, res) => {
  ...
  `;
    ...
    const results = await new Promise((resolve, reject) => {
      connection.query(get_user_id, [doctor_id], (err, result) => {
        if (err) {
          console.log("Error getting Doctor user_id", err);
          reject(err);
        } else {
...
app.post("/admin/delete_user", async (req, res) => {
  ...
  `;
    ...
    const results = await new Promise((resolve, reject) => {
      connection.query(delete_user, [user_id], (err, result) => {
        if (err) {
          console.log("Error getting Doctor user_id", err);
          reject(err);
        } else {
